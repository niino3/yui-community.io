...
export default function EquipmentDetail({ goBack, params }) {
  const [step, setStep] = useState('detail') // 'detail', 'reserve', 'confirmed', 'using', 'return'
  const [selectedSlot, setSelectedSlot] = useState('')
  const [rating, setRating] = useState(0)
  const [returned, setReturned] = useState(false)

  const item = equipment.find(e => e.id === params?.equipmentId) || equipment[0]
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-8 text-center">
          ...
          <div className="card w-full p-4 mt-2 text-left space-y-2">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-bold text-red-500">-{item.pricePerHalf} TOKEN</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-500">保証金返却</span>
              <span className="font-bold text-green-600">+{item.deposit} TOKEN</span>
            </div>
          </div>
        ...
        <div className="flex-1 px-4 py-4 space-y-4">
          ...
          <div className="card p-4 space-y-2">
            {[
              ...
              <button key={item.label} className="w-full flex items-center gap-3 py-3 px-3 bg-gray-50 rounded-xl text-sm font-medium text-gray-700 active:bg-gray-100">
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-8 text-center">
          ...
          <div className="card w-full p-4 text-left space-y-2">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-bold text-token-600">{item.pricePerHalf + item.deposit} TOKEN ロック中</span>
            </div>
          </div>
        ...
        <div className="flex-1 px-4 py-4 space-y-4">
          ...
          <div className="card p-4">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">利用料</span>
                <span className="font-bold text-token-600">{item.pricePerHalf} TOKEN</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">保証金（返却時に返却）</span>
                <span className="font-bold text-gray-600">{item.deposit} TOKEN</span>
              </div>
              <div className="border-t border-gray-100 pt-2 flex justify-between text-sm">
                <span className="font-bold text-gray-700">合計（一時ロック）</span>
                <span className="font-black text-token-600">{item.pricePerHalf + item.deposit} TOKEN</span>
              </div>
            </div>
          </div>
        </div>
    ...
    <div className="screen">
        ...
        <div className="px-4 py-4 space-y-3">
          <div className="card p-4">
            <div className="grid grid-cols-3 gap-3 text-center">
              <div>
                <p className="text-xl font-black text-token-600">{item.pricePerHalf}</p>
              ...
              <div>
                <p className="text-xl font-black text-gray-700">{item.useCount}</p>
          ...
          <div className={`card p-4 ${item.available ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}`}>
            <p className={`text-sm font-bold ${item.available ? 'text-green-700' : 'text-red-600'}`}>
              {item.available ? '✅ 利用可能' : '❌ 貸出中'}
            </p>
            <p className="text-xs text-gray-500 mt-1">{item.nextAvailable}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-shrink-0 p-4 border-t border-gray-100">
        <button
          ...
          className={`btn-primary ${!item.available ? 'opacity-50' : ''}`}
        >
          {item.available ? '予約する' : '貸出中 — 予約不可'}
        </button>
      </div>
    </div>
  )
